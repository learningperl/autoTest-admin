...
public class Mysql {
	...
	public Mysql() {	//Mysql地址配置
		try {
			...
			System.out.println(property.readRcErpURL("mysqlPath"));
			ct = DriverManager.getConnection(property.readRcErpURL("mysqlPath"), property.readRcErpURL("mysqlu"), property.readRcErpURL("mysqlp"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
